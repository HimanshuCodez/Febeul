...
const DynamicPolicy = () => {
  ...
  useEffect(() => {
    ...
    const fetchPolicy = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${backendUrl}/api/policy/${policyName}`);
        setPolicyContent(response.data.content || []);
        setPageTitle(response.data.pageTitle || policyName);
        setError('');
      } catch (err) {
        console.error(`Error fetching policy ${policyName}:`, err);
        setError('Failed to load this policy. Please try again later.');
        setPageTitle('Error');
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fde2e4] to-[#f9d1d1] py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        {policyContent && policyContent.length > 0 && (
          <motion.div
            variants={{
              hidden: { opacity: 0, y: 40 },
