...
const backendUrl = import.meta.env.VITE_BACKEND_URL; // Adjust path if needed
        ...
        <div className="space-y-4">
          {content.map((item, idx) => {
            switch (item.type) {
              ...
              case 'list':
                // The new admin panel might send individual list items as 'list' type with 'text'
...
const LuxePolicy = () => {
  ...
  const [pageTitle, setPageTitle] = useState('LUXE MEMBERSHIP POLICY');
  ...
  useEffect(() => {
    ...
    const fetchPolicy = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/policy/LuxePolicy`);
        if (response.data) {
          setPolicyData(response.data.content);
          setPageTitle(response.data.pageTitle);
        }
      } catch (err) {
        console.error('Error fetching Luxe Policy:', err);
      } finally {
        setLoading(false);
      }
    };
