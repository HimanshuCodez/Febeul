...
const GrievanceRedressals = () => {
  ...
  const [pageTitle, setPageTitle] = useState('Loading Grievance Redressal Policy...');
  ...
  useEffect(() => {
    ...
    const fetchPolicy = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/policy/GrievanceRedressals`); // Assuming policyName "GrievanceRedressals"
        setPolicyContent(response.data.content);
        setPageTitle(response.data.pageTitle);
      } catch (err) {
        console.error('Error fetching Grievance Redressal policy:', err);
        setError('Failed to load Grievance Redressal policy. Please try again later.');
        setPageTitle('Error');
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#fde2e4] to-[#f9d1d1]">
        <p className="text-xl text-[#f47b7d] font-semibold">Loading Grievance Redressal Policy...</p>
      </div>
    );
  }
