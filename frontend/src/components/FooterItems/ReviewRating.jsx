...
const ReviewRating = () => {
  ...
  const [pageTitle, setPageTitle] = useState('Febeul Review & Rating Policy');
  ...
  useEffect(() => {
    ...
    const fetchPolicy = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/policy/ReviewRating`);
        if (response.data) {
          setPolicyData(response.data.content);
          setPageTitle(response.data.pageTitle);
        }
      } catch (err) {
        console.error('Error fetching Review/Rating Policy:', err);
      } finally {
        setLoading(false);
      }
    };
