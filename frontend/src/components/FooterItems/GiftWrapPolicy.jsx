...
const GiftWrapPolicy = () => {
  ...
  const [pageTitle, setPageTitle] = useState('Loading Gift Wrap Policy...');
  ...
  useEffect(() => {
    ...
    const fetchPolicy = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/policy/GiftWrapPolicy`); // Assuming policyName "GiftWrapPolicy"
        setPolicyContent(response.data.content);
        setPageTitle(response.data.pageTitle);
      } catch (err) {
        console.error('Error fetching Gift Wrap policy:', err);
        setError('Failed to load Gift Wrap policy. Please try again later.');
        setPageTitle('Error');
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#fde2e4] to-[#f9d1d1]">
        <p className="text-xl text-[#f47b7d] font-semibold">Loading Gift Wrap Policy...</p>
      </div>
    );
  }
