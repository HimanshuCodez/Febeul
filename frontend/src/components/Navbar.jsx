...
import SwipingMessages from "./SwippingMsgs";
                                    ...
                                    <div key={catIndex}>
                                      ...
                                      <ul className="space-y-1">
                                        {links.map((link, linkIndex) => (
                                          <li key={linkIndex}>
                                            <Link
                                              to={`/products/${item.title.replace(/ /g, "-").replace("🎁", "").toLowerCase()}/${category.toLowerCase()}/${link.toLowerCase().replace(/'/g, '-').replace(/[^a-z0-9- ]/g, '').replace(/ /g, "-")}`}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navigation.map((item, index) => (
              <div key={index}>
                ...
                {item.megaMenu && openMegaMenus[index] && ( // Conditionally render megaMenu content
                  <div className="pl-4 pb-2"> {/* Added pb-2 for spacing */}
                    {Object.entries(item.megaMenu).map(
                      ([category, links], catIndex) => (
                        <div key={catIndex}>
