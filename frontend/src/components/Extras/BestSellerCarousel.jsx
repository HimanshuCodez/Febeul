  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to load best sellers.');
      }
    } catch (err) {
    ...
    return (
      <div className="w-full rounded-3xl bg-white/80 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, index) => (
            <div key={index} className={`space-y-3 animate-pulse ${index === 2 ? 'hidden sm:block' : ''}`}>
    ...
    return (
      <div className="flex min-h-[320px] items-center justify-center rounded-3xl border border-red-100 bg-red-50 p-6 text-center">
        <div>
          ...
          <h3 className="text-xl font-semibold text-red-900">Unable to load best sellers</h3>
  ...
  if (products.length === 0) {
    return (
      <div className="flex min-h-[320px] items-center justify-center rounded-3xl border border-gray-100 bg-white p-6 text-center">
        <div>
          <h3 className="text-xl font-semibold text-gray-900">No bestseller products found</h3>
          <p className="mt-2 text-sm text-gray-500">Check back later for featured items.</p>
        </div>
      </div>
    );
  }
        ...
        >
          {loopedProducts.map((product, index) => (
            <div
              ...
              className="flex-none w-[calc(50%-8px)] sm:w-[320px] lg:w-[340px] snap-start"
            >
