          ...
          {addresses.map((address) => {
            ...
            return (
              <div
                ...
                className={`p-4 border rounded-lg transition-all min-w-0 ${!isServiceable ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'} ${
              ...
              >
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <p className="font-semibold break-words">{address.name}</p>
                  ...
                  {!isServiceable && (
                    <span className="px-2 py-0.5 bg-red-100 text-red-600 text-[10px] rounded-full uppercase font-bold border border-red-200 shrink-0">
                        Not serviceable
                    </span>
                  )}
                </div>
                <p className="break-words">{address.address}, {address.locality}</p>
                {address.landmark && <p className="text-sm text-gray-500 italic break-words">Landmark: {address.landmark}</p>}
                <p className="break-words">{address.zip}, {address.city}</p>
                <p className="break-words">{address.state}, {address.country}</p>
                <p className="break-words">Phone: {address.phone}{address.alternatePhone ? `, ${address.alternatePhone}` : ''}</p>
              </div>
            );
          })}
        </div>
