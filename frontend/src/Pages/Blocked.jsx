  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[#1a0b0c] flex items-center justify-center p-4 overflow-y-auto">
      <motion.div
        ...
        className="relative max-w-lg w-full bg-white rounded-3xl shadow-2xl p-6 sm:p-8 my-8"
      >
        ...
        <AnimatePresence mode="wait">
          {justSubmitted ? (
            ...
            <motion.form
              ...
              initial={{ opacity: 0, height: 0 }}
              animate={{ opacity: 1, height: 'auto' }}
              className="mt-5 space-y-4 p-5 bg-slate-50/60 border border-slate-100 rounded-2xl overflow-hidden"
            >
              ...
              <div className="space-y-2">
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-1">Attach Photos (Optional, Max 2)</label>
                {images.length < 2 && (
                  <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-slate-200 rounded-xl p-5 cursor-pointer hover:border-pink-300 hover:bg-pink-50/40 transition-colors text-center">
                ...
                {images.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {images.map((image, index) => (
                      <div key={index} className="relative w-16 h-16 border border-slate-200 rounded-xl overflow-hidden shadow-sm">
