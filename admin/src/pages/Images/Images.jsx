...
const Images = ({ token }) => {
  ...
  const handleImageUpload = async (type, index, file) => {
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto">
      <h2 className="text-3xl font-bold mb-8 text-gray-800">Visual Content Management</h2>
      ...
      <div className="mb-4 border rounded-lg overflow-hidden shadow-sm">
        <button 
          ...
          className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors font-semibold text-lg"
        >
        ...
        {activeAccordion === 'hero' && (
          <div className="p-6 bg-white space-y-6">
              ...
              <div key={index} className="border p-4 rounded-md relative bg-gray-50">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1">
                    <p className="mb-2 font-medium">Slide Image {index + 1}</p>
                    <div className="flex items-center gap-4">
                      {slide.image ? (
                        <img src={slide.image} alt="" className="w-40 h-24 object-cover rounded border" />
                      ) : (
                        <div className="w-40 h-24 bg-gray-200 flex items-center justify-center border rounded text-gray-400">No Image</div>
                      )}
                      <label className="cursor-pointer bg-white border border-gray-300 px-4 py-2 rounded hover:bg-gray-50 transition shadow-sm">
                        Upload
                        <input type="file" className="hidden" onChange={(e) => handleImageUpload('hero', index, e.target.files[0])} />
                      </label>
                    </div>
                  </div>
                  <div className="flex-1">
            ...
            <div className="flex gap-4">
              <button onClick={() => setSlides([...slides, { image: '', link: '' }])} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition">Add Slide</button>
              <button onClick={() => saveContent('hero_carousel', slides)} className="bg-black text-white px-8 py-2 rounded hover:bg-gray-800 transition font-bold">Save Hero Carousel</button>
            </div>
          </div>
        )}
      </div>
        ...
        {activeAccordion === 'spotlight' && (
          <div className="p-6 bg-white space-y-6">
              ...
              {spotlight.map((item, index) => (
                <div key={index} className="border p-4 rounded-md relative bg-gray-50">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      {item.image ? (
                        <img src={item.image} alt="" className="w-20 h-20 rounded-full object-cover border" />
                      ) : (
                      ...
                      <label className="cursor-pointer bg-white border border-gray-300 px-3 py-1 text-sm rounded hover:bg-gray-50 shadow-sm">
                        Upload Image
                        <input type="file" className="hidden" onChange={(e) => handleImageUpload('spotlight', index, e.target.files[0])} />
                      </label>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-sm font-medium mb-1">Redirect Link (Optional)</p>
                      <input
                        type="text"
            ...
            <div className="flex gap-4 pt-4 border-t">
              <button onClick={() => setSpotlight([...spotlight, { image: '', label: '', link: '' }])} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition">Add Category</button>
              <button onClick={() => saveContent('spotlight_categories', spotlight)} className="bg-black text-white px-8 py-2 rounded hover:bg-gray-800 transition font-bold">Save Spotlight</button>
            </div>
          </div>
        )}
      </div>
