...
const RefundRequests = ({ token }) => {
  ...
  const handleApproveRefund = async (orderId) => {
    if (!window.confirm("Are you sure you want to approve and process this automated refund? This will trigger the payment gateway (Razorpay) if applicable.")) return;
            ...
            <div className="p-8 border-t border-gray-100 bg-gray-50 flex flex-col gap-4">
                {selectedRequest.refundDetails.status !== 'completed' && (
                    <div className="flex flex-col gap-3 p-4 bg-white rounded-3xl border border-gray-200 shadow-sm">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Administrative Action (Automated)</p>
                        <div className="flex gap-2">
                            <select 
                                value={selectedReason}
                                onChange={(e) => setSelectedReason(e.target.value)}
                                className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:border-pink-500 transition-all"
                            >
                                <option value="">Auto-calculate Reason</option>
                                <option value="buyer_fault">Buyer Fault (Deduct 150)</option>
                                <option value="seller_fault">Seller Fault (Full Refund)</option>
                                <option value="courier_fault">Courier Fault (Full Refund)</option>
                            </select>
                            <button 
                                onClick={() => handleApproveRefund(selectedRequest._id)}
                                disabled={isUpdating}
                                className="bg-pink-600 text-white px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-pink-700 transition-all disabled:opacity-50"
                            >
                                Approve & Refund
                            </button>
                        </div>
