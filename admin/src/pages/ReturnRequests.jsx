...
// from RefundRequests.jsx so admins have a dedicated queue for reverse-pickup
// tracking, separate from cancellations/money-only refunds/courier RTOs.
const ReturnRequests = ({ token }) => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setAllOrders(orders);
        const returnOrders = orders.filter(order => order.refundDetails?.requestType === 'return');
        setRequests(returnOrders.sort((a, b) => new Date(b.refundDetails.requestedAt || b.date) - new Date(a.refundDetails.requestedAt || a.date)));
      }
    } catch (error) {
  ...
  const handleApproveRefund = async (orderId) => {
    ...
    if (!window.confirm(`Are you sure you want to approve and process this ${isPartial ? 'partial' : 'full'} refund? This will trigger the payment gateway (Razorpay) for prepaid orders and schedule a reverse pickup.`)) return;
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Return Requests</h2>
          <p className="text-gray-500 font-medium">Customer returns of physically delivered items — reverse pickup + refund</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Pickup</th>
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              {loading ? (
                ...
                <tr><td colSpan="6" className="px-6 py-20 text-center text-gray-400 font-medium">No return requests found.</td></tr>
              ) : (
                  ...
                  <tr key={req._id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${
                        req.refundDetails.pickup?.status === 'delivered_to_warehouse' ? 'bg-emerald-50 text-emerald-600' :
                        req.refundDetails.pickup?.status === 'failed' ? 'bg-amber-50 text-amber-600' :
                        req.refundDetails.pickup?.status && req.refundDetails.pickup.status !== 'none' ? 'bg-blue-50 text-blue-600' :
                        'bg-gray-50 text-gray-500'
                      }`}>
                        {pickupStatusLabels[req.refundDetails.pickup?.status || 'none']}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[100] flex justify-end">
          <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div>
                <h3 className="text-2xl font-black text-gray-900 tracking-tighter uppercase">Return Details</h3>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                {selectedRequest.paymentMethod === 'Razorpay' ? (
                  <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 space-y-2">
                    ...
                    <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100 space-y-4">
                        <p className="text-[10px] font-black text-emerald-600 uppercase">COD Settlement Details ({selectedRequest.refundDetails.customerPayoutDetails.type === 'upi' ? 'UPI' : 'Bank Transfer'})</p>
                        {selectedRequest.refundDetails.customerPayoutDetails.type === 'upi' ? (
                          <div><p className="text-[10px] font-black text-emerald-600 uppercase">UPI ID</p><p className="text-lg font-black tracking-widest font-mono">{selectedRequest.refundDetails.customerPayoutDetails.upiId}</p></div>
                        ) : (
              ...
              <section>
                ...
                <div className={`p-6 rounded-3xl border space-y-2 ${
                  selectedRequest.refundDetails.pickup?.status === 'failed' ? 'bg-amber-50 border-amber-100' :
                  selectedRequest.refundDetails.pickup?.status === 'delivered_to_warehouse' ? 'bg-emerald-50 border-emerald-100' :
                  ...
                  <p className={`text-[10px] font-black uppercase ${selectedRequest.refundDetails.pickup?.status === 'failed' ? 'text-amber-600' : 'text-orange-600'}`}>
                    {pickupStatusLabels[selectedRequest.refundDetails.pickup?.status || 'none']}
                  </p>
                  {selectedRequest.refundDetails.pickup?.status === 'failed' ? (
                  ...
                  ) : selectedRequest.refundDetails.pickup?.status && selectedRequest.refundDetails.pickup.status !== 'none' ? (
                    <>
                      ...
                      {selectedRequest.refundDetails.pickup.trackingHistory?.length > 0 && (
                        <div className="pt-2 mt-2 border-t border-orange-100 space-y-1">
                          {[...selectedRequest.refundDetails.pickup.trackingHistory].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 5).map((act, i) => (
                    ...
                    <p className="text-xs text-gray-600">Pickup will be scheduled automatically once this return is approved.</p>
                  )}
                </div>
              </section>
            </div>
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex gap-2 pt-2 border-t border-gray-100">
                              ...
                              <button onClick={() => handleApproveRefund(selectedRequest._id)} disabled={isUpdating} className="bg-pink-600 text-white px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-pink-700 disabled:opacity-50 flex-1">
                                  {selectedRequest.paymentMethod === 'Razorpay' ? 'Pay, Refund & Schedule Pickup' : 'Approve & Schedule Pickup'}
                              </button>
